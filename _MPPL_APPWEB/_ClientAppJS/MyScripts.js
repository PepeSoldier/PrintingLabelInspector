...
function InitAreaAutocompleteByName(elementSelector, selectorForName, selectorForValue) {
    $(elementSelector).autocomplete({
        source: function (request, response) {
            $.ajax({
                url: "/PFEP/Area/AutocompleteByName",
                type: "POST",
                dataType: "json",
                data: { Prefix: request.term },
                success: function (data) {
        ...
        select: function (event, ui) {
            $(selectorForName).val(ui.item.label);
            $(selectorForValue).val(ui.item.valuer);
            return false;
        },
        messages: {
            noResults: "", results: function () { }
...
function InitPackageAutocompleteByCode(elementSelector, selectorForName, selectorForValue) {
    $(elementSelector).autocomplete({
        source: function (request, response) {
            $.ajax({
                url: "/PFEP/Package/AutocompleteByCode",
                type: "POST",
                dataType: "json",
                data: { Prefix: request.term },
                success: function (data) {
        ...
        select: function (event, ui) {
            ...
            console.log(ui);
            $(selectorForName).val(ui.item.label);
            $(selectorForValue).val(ui.item.labelName);
            return false;
        }
    });
}
function InitMachineAutocompleteByName(elementSelector, selectorForName, selectorForValue) {
    $(elementSelector).autocomplete({
        source: function (request, response) {
            $.ajax({
                url: "/ONEPROD/OEE/AutocompleteByName",
                type: "GET",
                dataType: "json",
                data: { Prefix: request.term },
                success: function (data) {
...
function InitAddActionToMeeting() {
    $('.AddToMeeting').click(function () {
        var url = window.location.pathname;
        var id = this.attributes[4].value;
        var meetingId = this.attributes[5].value;
        $.ajax({
            url: baseUrl + "Meeting/AddAction",
            type: "POST",
            dataType: "json",
            data: { ActionId: id, MeetingId: meetingId },
            success: function () {
                location.reload();
            }
        });
...
//SPCIFIC FOR PACKING INSTR. GRID. SHOULDN't BE HERE!
function PackingInstructionJsGridDefaults() {
    jsGrid.setDefaults({
        height: "880px", width: "100%",
        autoload: true,
        paging: true, pageLoading: true, pageSize: 100, pageIndex: 1, pageButtonCount: 5,
        inserting: false, filtering: true, sorting: true,
...
function InitCorrectionField(link) {

    var MyCorrectionField = function (config) {
        jsGrid.Field.call(this, config);
    };

    MyCorrectionField.prototype = new jsGrid.Field({

        itemTemplate: function (value, item) {
            var k = '<a href="' + link + 'PFEP/PackingInstruction/Show/' + item.Id + '" data-toggle="tooltip" title="Zgłoś Poprawkę">' +
                '<div class ="btn btn-warning btn-sm"><span class="glyphicon glyphicon-edit">' +
                '</div>' +
                '</a>';
            return k;
        },
    ...
    return MyCorrectionField;
}
function InitPrintField(link) {

    var MyPrintField = function (config) {
        jsGrid.Field.call(this, config);
    };

    MyPrintField.prototype = new jsGrid.Field({

        itemTemplate: function (value, item) {
            var k = '<a href="' + link + 'PFEP/PackingInstruction/SavePDF/' + item.Id + '" data-toggle="tooltip" title="Pobierz PDF">' +
                '<div class ="btn btn-primary btn-sm"><span class="fas fa-file-pdf">' +
                '</div>' +
                '</a>';
            return k;
        },
...
function InitViewField(link) {

    var MyViewField = function (config) {
        jsGrid.Field.call(this, config);
    };

    MyViewField.prototype = new jsGrid.Field({

        itemTemplate: function (value, item) {
            var k = '<a href="' + link + 'PFEP/PackingInstruction/Show/' + item.Id + '" data-toggle="tooltip" title="Podgląd">' +
                '<div class ="btn btn-default btn-sm"><span class="fas fa-arrow-alt-circle-right">' +
                '</div>' +
                '</a>';
            return k;
        },
...
//--------------ACTION PLAN CREATE. SHOULD't BE HERE!
function FillComboLineAndWorkst(masterComboId, LineComboId, WorkstationComboId) {

    var relatedObjId = $(masterComboId).val();

    console.log("change " + relatedObjId);

    $(LineComboId).html("");
    $(LineComboId).append($('<option></option>'));

    $(WorkstationComboId).html("");
    $(WorkstationComboId).append($('<option></option>'));

    if (relatedObjId > 0)
    {
        $.ajax({
            url: baseUrl + 'AP/Def/GetLines',
            type: "POST",
            dataType: "JSON",
            data: { id: relatedObjId },
        ...
        $.ajax({
            url: baseUrl + 'AP/Def/GetWorkstations',
            type: "POST",
            dataType: "JSON",
            data: { id: relatedObjId },
            success: function (objects) {
                $.each(objects, function (i, obj) {
                    $(WorkstationComboId).append(
                        $('<option></option>').val(obj.Id).html(obj.Name));
                });
            }
        });
...
function onChangeDropClearAssigner() {
    $(".dropClearAssigner").change(function () {
        $(".UserTextBox").val('');
        $('#NewObject_AssignedId').val('');
    });
}
//----------------------------------------------------
