...
JustGage = function (config) {
    ...
    obj.config = {
        ...
        //target (kamil 2019-01-29)
        target: kvLookup('target', config, dataset, 0, 'float'),
        targetColor: kvLookup('targetColor', config, dataset, '#444444'),
    ...
    obj.levelTarget = obj.canvas.path().attr({
        "stroke": "none",
        "fill": obj.config.targetColor,
        pki: [
            obj.config.target,
            obj.config.min,
            obj.config.max,
    ...
    if (obj.config.donut) {
        ...
        obj.levelTarget.transform("r" + obj.config.donutStartAngle + ", " + (obj.params.widgetW / 2 + obj.params.dx) + ", " + (obj.params.widgetH / 2 + obj.params.dy));
    }
...
/** Put Target (Kamil 2019-01-29) */
JustGage.prototype.putTarget = function (targetVal, targetColor) {
    ...
    if ((targetVal * 1) > (obj.config.max * 1)) {
        targetVal = (obj.config.max * 1);
    }
    if ((targetVal * 1) < (obj.config.min * 1)) {
        targetVal = (obj.config.min * 1);
    }

    color = targetColor;
    obj.config.target = displayVal * 1;
    obj.config.targetColor = targetColor;
    
    var rvl = obj.config.target;
    if (obj.config.reverse) {
        rvl = (obj.config.max * 1) + (obj.config.min * 1) - (obj.config.value * 1);
    }
    obj.levelTarget.animate({
        ...
        "fill": color
    }, obj.config.refreshAnimationTime*0, obj.config.refreshAnimationType, obj.config.onAnimationEnd);
